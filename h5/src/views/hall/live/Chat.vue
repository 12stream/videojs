...
<div class="chatInfo">
    <div class="chat-bottom">
        ...
        <button @click="send" :disabled='chat.chatFlag' :class="chat.chatFlag ? 'stop' : ''">发送</button>
    </div>
    <div class="chat-gift">
        <div class="gift invite" @click="invite"></div>
        <div id="btn1"></div>
        <button class="gift praise" @click="praise">
    ...
    <div :class="ctime" id="chat-message">
        <div class="wrapper">
            ...
            <ul class="chat-list-audience" id="chat-message12">
                <li class="chat-list" v-for="(item,i) in chat.MsgData" :key="i">
                    ...
                    <div class="audience">
                        ...
                        <span class="audience-word" v-else-if="item.cmd == 1001">进入房间</span>
                    </div>
                </li>
            </ul>
            </div>
     ...
     <!-- 礼物 -->
        ...
        <Wait class="chatF" v-if="chatFL"></Wait>
        <div class="mask-chatF" v-if="chatMF"></div>
</div>
</template>
<script>
...
import {timestampToTime, setLStorage, getLStorage, getCookie, connetChat} from '@/api/config'
...
export default {
    ...
    props:['detail', 'wxQQ', 'cdtime', 'ctime'],
    components:{videoPlayer, Login, Wait},
    methods:{
        // 点击遮罩层关闭绑定手机号登录框
        close() {
            this.flag = false;
        ...
        send() {
            ...
            if((!getLStorage('mobile') || getLStorage('mobile')  == 'undefined') && !is_weixn()) {
                ...
                if(this.socket && this.chat.MsgTxt != "" && !this.chat.MsgTxt.match(/^[ ]+$/)) {
                        ...
                        console.log('提前打印情况',getLStorage('wxUserData'),getLStorage('mobileUserInfo'))
                        if (getLStorage('wxUserData') && getLStorage('wxUserData')　!== 'undefined') {
                            var mes = JSON.parse(getLStorage('wxUserData'))
                            console.log(mes)
                            var name = mes.nickname
                            var headImg = mes.headImg
                        }else if(getLStorage('mobileUserInfo') && getLStorage('mobileUserInfo') !== 'undefined') {
                            var mobileMes = JSON.parse(getLStorage('mobileUserInfo'))
                            console.log(mobileMes)
                        ...
                        console.log('来看看',mes,mobileMes,getCookie('wxuserid') || getLStorage('userid'))
                        this.socket.emit("msg", {
                        imgUrl: headImg ? headImg : this.chat.imgUrl,
                        cmd: 1004,
                        msg: "弹幕",
                        nickName: name ? name : phoneNumber, //nickname,
                        user_id: getCookie('wxuserid') || getLStorage('userid'),
                        barrage: parseInt(0),
                        data: {
        ...
        // 创建直播间
        createLiveRoom() {
             if(getLStorage('thirdUserInfo')&& is_weixn()) {
                 var thirInfo = JSON.parse(getLStorage('thirdUserInfo'))
                 var userId = thirInfo.id
                 var enterName = thirInfo.user_title
                 var enterImg = thirInfo.user_icon_url
             }else{
                if(getLStorage('mobileUserInfo')){
                    var mobileMes = JSON.parse(getLStorage('mobileUserInfo'))
                    if(mobileMes.user_title){
                        var enterName = mobileMes.user_title
                        var enterImg = mobileMes.user_icon_url
                    }else{
                        var enterName = getLStorage('mobile').replace(/(\d{3})(\d{4})(\d{4})/, "$1****$3")
                    }
                }else{
                    var enterName = getLStorage('mobile').replace(/(\d{3})(\d{4})(\d{4})/, "$1****$3")
                }
            ...
            if (this.chat.isSocket) {
                ...
                this.chat.user = {
                    ...
                    nickName: enterName || '游客', //nickname,
                    user_id: userId, //用户ID    类型：整形
                    imgUrl:enterImg || this.chat.imgUrl || this.chat.portrait, //用户头像链接 类型：字符串
                    gender: 1 ,//性别
                    device:"portal"
                };
                ...
                this.$ajax.changeLiveRoom(params).then(res =>{
                    ...
                    this.chat.out_room_ServerAddress = connetChat
            ...
            this.$nextTick(() =>{
                ...
                this.socket.on("connect",() =>{
                    if(getLStorage('thirdUserInfo')&& is_weixn()) {
                        ...
                        var userIdC = getLStorage('userid')
                    }
                    this.chat.user = {
                        cmd: 1001, //命令号
                        roomID: this.chat.out_roomID,
                        nickName: enterName || '游客', //nickname,
                        user_id: userIdC, //用户ID    类型：整形
                        imgUrl: enterImg || this.chat.imgUrl, //用户头像链接 类型：字符串
                        gender: 1, //性别
                        device:"portal"
                    };
                    this.socket.emit("join", this.chat.user);
                ...
                this.socket.on("reconnecting", () => {
                    ...
                    this.chat.user = {
                        ...
                        nickName: enterName,
                        user_id: userIdC,
                        imgUrl: enterImg || this.chat.imgUrl,
                        gender: 1,
                        device:"portal"
                    };
                    this.socket.emit("join", this.chat.user);
                ...
                this.socket.on('sys', sysMsg =>{
                    ...
                    case 1001:
                        ...
                        var enterRoom = this.chat.Msg
                        if(enterRoom.cmd == 1001 && enterRoom.msg == '加入房间' && enterRoom.user_id !== '1'){
                            this.chat.MsgData.push(enterRoom);
                        }
                        if(this.chat.Msg.data.silence_all == 1) {
                            this.$toast('全员禁言哦~')
                            this.userWordMessage = '全员禁言，仅允许管理员发言'
                            this.chat.chatFlag = true
                    ...
                    case 2002:
                        ...
                        this.chat.Msg.send_time = timestampToTime(this.chat.Msg.send_time,1);
                        console.log(this.chat.Msg)
                        eventVue.$emit('liveS',this.chat.Msg.data.live_statu)
                        break;
                    case 1004:
                        // 聊天室通用消息
                        if (this.socket) {
        ...
        sendGift(gid) {
            ...
            setLStorage('giftId',gid.id)
            setLStorage('giftmoney',(gid.giveCurrencyValue/100).toFixed(2))
            this.chat.MsgGiftata = gid.giftName;
            this.chat.MsgImg = gid.giftImg;
            setLStorage('chatGift',gid.giftImg)
        ...
        selfSendGift() {
            if(this.socket) {
                ...
                if(getLStorage('wxUserData')) {
                    ...
                    if(getLStorage('thirdUserInfo')) {
                        var thirInfo = JSON.parse(getLStorage('thirdUserInfo'))
                        var chatUserId = thirInfo.id
                    }
                }
                console.log(JSON.stringify(mes),chatImg,nickName)
                this.socket.emit("msg", {
                    ...
                    giftUrl:this.chat.MsgImg || getLStorage('chatGift'),
                    barrage: parseInt(0),
                    data: {
                ...
                this.$ajax.list({receiveLiveID:detail.id}).then(res =>{
                    // console.log(res)
                    this.listNumber = res.params.totalAmount-0
                    if (res.params.page > 8) {
                        var listR = res.params.list.slice(0,7)
                    }else {
                        this.listR = res.params.list
                    }
                    eventVue.$emit('listD',listR)
                })
        },
        // 赠送支付
        paySend() {
            if(this.chooseGiftFlag) {
                if(getLStorage('thirdUserInfo')){
                    console.log(getLStorage('thirdUserInfo'))
                    var thirMes = JSON.parse(getLStorage('thirdUserInfo'))
                    if(!thirMes.mobile){
                        this.flag = true
                        this.loginState = 'changF'
                        this.giftFlag = false
        ...
        weixinPay() {
            ...
            if(getLStorage('thirdUserInfo')){
                var mesThir = JSON.parse(getLStorage('thirdUserInfo'))
                var thirUserid = mesThir.id
                if(mesThir.mobile){
                    var thirMobile = mesThir.mobile
                }
            }
            var params = {
                userId:getLStorage('userid') || thirUserid,
                userName:mes.nickname,
                open_id:getCookie('wxUserOpenid'),
                gender:mes.sex,
                address:'',
                phoneNumber:getLStorage('mobile') || thirMobile,
                imgUrl:mes.headImg,
                receiveUserID:1,
                receiveLiveID:this.detail.id,
            ...
            this.$ajax.giftPayStart(params).then(res =>{
                ...
                wx.config({
                    ...
                    this.selfSendGift()
                    this.$ajax.list({receiveLiveID:detail.id}).then(res =>{
                        console.log('微信支付2次确认结果榜单',JSON.stringify(res))
                         if (res.params.page > 8) {
                            var listR = res.params.list.slice(0,7)
                        }else {
                            this.listR = res.params.list
        ...
        hfivePay() {
            setLStorage('roomId',this.detail.id)
            var wxMessage = getCookie('wxUserInfo')
            if(getLStorage('mobileUserInfo')){
                var mobileMes = JSON.parse(getLStorage('mobileUserInfo'))
                var fpUserid = mobileMes.id
                var fpMobile = mobileMes.mobile
            }
            // var newP = window.open('about:blank')
             var params = {
                userId:getLStorage('userid') || fpUserid,
                userName:getLStorage('mobile') || fpMobile,
                gender:'',
                address:'',
                phoneNumber:getLStorage('mobile') || fpMobile,
                imgUrl:'',
                receiveUserID:1,
                receiveLiveID:this.detail.id || getLStorage('roomId'),
                receiveVideoID:this.detail.id || getLStorage('roomId'),
                giftID:this.curGiftId || getLStorage('giftId'),
                giftCount:1,
                type:2,
                money:this.curGiftMoney || getLStorage('giftmoney'),
                open_id:'',
                source:2
            }
            console.log(params)
            this.$ajax.giftPayStart(params).then(res =>{
                ...
                setLStorage('giftId',res.data.orderNo)
                console.log(res.data.mweb_url)
                // alert(JSON.stringify(params))
                // alert(window.location.href)
                // alert(JSON.stringify(res))
                var curPUrl = window.location.href + '?payG=2'
                //  alert(`${res.data.mweb_url}&redirect_url=${encodeURIComponent(curPUrl)}`)
                 if(curPUrl == 'undefined' || curPUrl == undefined){
                     var curPUrl = getLStorage('currentUrl')
                 }else{
                     setLStorage('currentUrl',curPUrl)
                 }
        ...
        praise() {
            ...
            var index=$jqu('#btn1').children('img').length;
            var rand = parseInt(Math.random() * (x - y + 1) + y); 
            if(Math.random() > 0.5){
                var lef = parseInt(Math.random() * (z - u + 1) + u);
            }else {
                var lef = -parseInt(Math.random() * (z - u + 1) + u);
            }
            // console.log(rand,lef)
            $jqu("#btn1").append("<img src=''>");
            $jqu('#btn1 img:eq(' + index + ')').attr('src',require('../images/'+num+'.png'))
            $jqu('#btn1 img').css({
                position:'absolute',
                width:"30px",
                height:"30px",
            ...
            if(!getLStorage('mobile') && getLStorage('mobile')  !== 'undefined' && !is_weixn()) {
                ...
                if(!this.praiseFlag){
                     var params = {
                        ...
                        likeID:getLStorage('userid') || useridP,
                        liveID:this.$route.params.liveId
                    }
        ...
        payGS() {
            ...
            if(curGUrl.indexOf('payG') > -1) {
                ...
                this.$msgpay({
                    ...
                    var orderidP = getLStorage('giftId')
                     this.$ajax.giftPayState({orderNum:orderidP}).then(res =>{
                        ...
                        if(res.data == '1') {
                            ...
                            this.$msgbox({
                                ...
                                this.hfivePay()
                                 ...
                                 var orderidP = getLStorage('giftId')
                                 this.$ajax.giftPayState({orderNum:orderidP}).then(res =>{
                                     console.log('2次确认结果',JSON.stringify(res))
                                     if(res.data == '1') {
                                        this.$toast('支付成功!')
                                        this.selfSendGift()
                                        this.$ajax.list({receiveLiveID:detail.id}).then(res =>{
                                            console.log('2次确认结果榜单',JSON.stringify(res))
                                            if (res.params.page > 8) {
                                                var listR = res.params.list.slice(0,7)
                                            }else {
                                                this.listR = res.params.list
    ...
    mounted() {
        ...
        console.log('34355434545',this.detail.id,this.$route.params.liveId)
