    ...
    <div>
        ...
        <div class="showPhone">
          <div class="backgroundImg">
            <el-row class="wxShow">
            <el-col :span="24">
                <p class="shareTitle">{{serviceName}}</p>
            ...
            <el-col :span="24">
                <p class="smallTips">扫描联系客服进行咨询</p>
            </el-col>
            </el-row>
...
import { getLStorage ,getCookies,chargeError,facebac_URL} from "api/config";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    saveWx() {
      ...
      this.$ajaxs.setvideoServe(serveParams).then(data => {
        console.log(data);
        if (data.code == 1) {
          console.log(data);
          this.$message({
      ...
      this.$ajaxs.getVideoInfo(params).then(data => {
        ...
        if (data.code == 1) {
          if (data.result.envelope_enable == undefined) {
            ...
            if(data.result.service_name!=null&&data.result.service_name!=""){
              this.serviceName = data.result.service_name;
            }
            if(data.result.service_qr_url!=null&&data.result.service_qr_url!=""){
