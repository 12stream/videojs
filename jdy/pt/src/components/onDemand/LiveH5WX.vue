<template>
    <div style="height: 700px;">
        ...
        <div class="wxInfo">
            ...
            <p style="text-align: left;padding: 15px;padding-top: 0;">
            功能开关：
            ...
            <el-col :span="19">
                ...
                <el-row class="checkboxItem">
                    <el-checkbox v-model="scannerShow">进入点播间弹二维码</el-checkbox>
                </el-row>
                <el-row class="checkboxItem">
...
import { getLStorage,getCookies,facebac_URL } from "api/config";
export default {
  data() {
    return {
      wxDialog1:false,
      wxDialog2:false,
      wxImg:
        facebac_URL+"public/1178309224503852333/img/1_817433040746800104_1531107730.jpg",
      scannerShow: "0",
      focus: "0",
      wxName: "梦网云播",
      wxUsing: "0",
  ...
  methods: {
    ...
    getData(){
        ...
        if (this.userID) {
              Sparams = {
                  user_id: this.userID,
                  token: this.tkInfo,
                  param_id: getLStorage("defRoomId"),
                  param_type: "2"
                };
          }else{
            Sparams = {
              ...
              this.$ajaxs.getWXScanner(Sparams).then(data => {
                console.log(data);
                if (data.code == 1) {
                  this.wxImg = data.result.img_url; //"http://"+
      ...
      this.$ajaxs.getVideoInfo(params).then(data => {
        ...
        if (data.code == 1) {
          if (data.result.envelope_enable == undefined) {
            ...
            this.wxUsing = data.result.wechat_enable.toString();
            ...
            if (this.CreditType != 1) {
              ...
              // var Sparams={user_id:this.userID,param_id:getLStorage("defRoomId"),param_type:"2"}
              this.$ajaxs.getWXScanner(Sparams).then(data => {
                console.log(data);
                if (data.code == 1) {
                  this.wxImg=data.result.img_url;//"http://"+
    ...
    saveWx() {
      ...
      this.$ajaxs.setvideoWX(wxParams).then(data => {
        console.log(data);
        if (data.code == 1) {
          console.log(data);
          this.$message({
