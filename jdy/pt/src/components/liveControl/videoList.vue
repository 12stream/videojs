<template>
    <div class="video_table">
        ...
        <div class="bg-fff info-content" style="min-height:592px;">
            <div style="margin-bottom:50px;" class="btn_header">
                <!-- <el-button class="bgc_txt  btn_30" type="primary" @click="uploadVideo()">上传</el-button> -->
                <!-- <el-button class="bgc_txt  btn_30" type="primary" @click="getVideoLibrary()">添加</el-button> -->
                ...
                <ul v-if="videoList.isData" class="video_table video_list" style="min-height:452px;">
                    <el-checkbox-group v-model="checkList">
                        <li  v-for="(item,index) in videoList.data" class="item_box">                            
                                 <div class="item clear" style="margin-top: 23px;">
                                    <!-- <el-checkbox :label="item.video_id"></el-checkbox> -->
                                    ...
                                    <div class="item_info fl">
                                        <p class="length_txt">{{item.live_name}}</p>
                                    ...
                                    <div class="swi_check">
                                        ...
                                        v-model="item.video_switch"
                                        @change="soldVideo(item,item.video_switch)"
                                        >
                                        </el-switch>
                                    </div>
                                    <div  class="item_btnG">
                                        <el-button v-if="item.top==1" class="btn   btn_26 hover_red" @click="setTop(item,'0')">取消置顶</el-button>
                                        <el-button v-if="item.top!=1" class="btn   btn_26 hover_orange" @click="setTop(item,'1')">置顶</el-button>
        ...
        <el-dialog  
            ...
            <ul v-if="VideoLibrary.isData" class="myTable_list my_checkbox_list">            
                <el-checkbox-group v-model="videoCheckList">
                    <li v-for="item in VideoLibrary.data" class="item_box">
                        <el-checkbox :label="item.id"  @change="select_video">
                            <div class="item">
                                    <div class="item_img fl">
            ...
            <div v-if="!VideoLibrary.isData"  class="no_video_add"> 
                    <div class="no_data"></div>                                                                                                                       
                    <div class="tip_no_data">
                    <span class="default-color">当前无任何视频可进行添加！</span>
                </div>               
            </div>
            <span slot="footer" class="dialog-footer">
        ...
        <el-dialog
            ...
            <el-form :model="ruleFormEdit"  ref="ruleFormEdit"  label-position="right" label-width="88px" class="mt5">
                ...
                <el-form-item label="视频封面 :" prop="videoCover"  id="item_edit_cover"
                :rules="[
                            {required: true, message: '视频封面不能为空', trigger: 'blur'},                          
                        ]">
                    <el-upload
                ...
                <el-form-item label="录像标题 :"  prop="title" :rules="[
                    {required: true, message: '录像标题不能为空', trigger: 'blur'},
                ]">
...
<script>
    ...
    export default{
        ...
        mounted(){         
            // 视频上传   
            // this.uploadVideoForm.video = myVideoUrl
            // initUpload()  
        },
        computed:{
            resource(){
        ...
        methods:{  
            ...
            submitCheckList(){
                let _this = this
                let formData = new FormData()
                let url = DATA_URL + 'mwlive/live/add_videoInof.do'
                formData.append('video_id',_this.videoCheckList)
                formData.append('live_id',getLStorage("liveID"))               
                axios({
                    method: 'post',
                    url: url,
                    ...
                    if(res.data.code == ERR_OK){
                        ...
                        this.successMessage('添加失败','error')  
                    }
            ...
            imgUpload(file){
                ...
                axios({
                    ...
                    if(res.data.code == ERR_OK){
                        ...
                        $("#item_edit_cover .el-form-item__error").hide()               
                    }else{
                        this.successMessage('请选择图片！','error')
                    }
                }).catch((err) => {
            ...
            //     axios({
            //         method: 'post',
            //         url: DATA_URL + 'mwlive/live/select_videoInfoListPage.do',
            //         data:formdata,
            //         headers: {'Content-Type': 'multipart/form-data'}
            //     }).then((res) => {
                ...
                axios({
                    ...
                    if(res.data.code == '200'){
                        if(res.data.data.data_list == null||res.data.data.data_list==""){
                            ...
                            for(let i = 0;i < data.length;i++){
                                if (data[i].video_switch== 1) {
                                    data[i].video_switch = false 
                                }else{
                                    data[i].video_switch = true 
                                }
                            }                            
                            this.tableData = data
                            this.videoList.data = data  
            ...
            setTop(item,type){
                ...
                axios({
                    ...
                    if(res.data.code == ERR_OK){
                        if(type=="0"){
                             _this.successMessage('已取消置顶')
                        }else{
                            _this.successMessage('已置顶')
                        }
            ...
            submitEdit(){
                ...
                editDate.append('video_name',this.ruleFormEdit.title)
            ...
            delOk(){
                ...
                axios({
                        method: 'post',
                        url: DATA_URL + 'mwlive/live/record_delete.do',    //delete_videoInfo.do',
            ...
            delVideoMore(){
                let _this = this
                let formdata = new FormData()
                formdata.append('video_id',this.checkList)   
                _this.dialogVisibleDel = true  
                _this.del_formdata =  formdata  
            ...
            openModify(type,formName,scope){
                ...
                if(type){
                    ...
                    // this.scope = scope
                    // this.ruleFormAdd.name = this.scope.name
                    // this.ruleFormAdd.phone = this.scope.phone
                    // this.ruleFormAdd = scope
