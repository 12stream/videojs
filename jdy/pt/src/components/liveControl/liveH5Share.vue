...
import { getLStorage, getCookies,chargeError,DATA_URL } from "api/config";
...
export default {
  ...
  methods: {
      ...
      axios
        ...
        .then(response => {
          ...
          if (response.status == "200") {
            this.shareImg = response.data.data.live_cover_url;
            this.shareTitle = response.data.data.live_name;
          }
        });
    },
    getImgUrl(a) {
    ...
    saveShare() {
      ...
      this.$ajaxs.setLiveEnvelope(serveParams).then(data => {
        console.log(data);
        if (data.code == 1) {
          console.log(data);
          this.$message({
      ...
      this.$ajaxs.getH5LiveInfo(params).then(data => {
        ...
        if (data.code == 1) {
          if (data.result.envelope_enable == undefined) {
            ...
            if (data.result.envelope_desc != null&&data.result.envelope_desc != null) {
              this.shareDescrib = data.result.envelope_desc;
            }
            if (data.result.envelope_logo_url != null&&data.result.envelope_logo_url != null) {
