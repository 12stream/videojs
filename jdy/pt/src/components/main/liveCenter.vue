<template>
    <div class="main live_center">
        ...
        <el-row class="w1000">
            <el-col class="mt20">
                <!-- <el-row class="mt20" v-loading.body="!dataList.length">
                ...
                <el-row>
                    <el-col  class="bg-fff default_shadow my_section">
                        <div class="my_liveT mw-subTitle">
                            <!-- <h3 class="col-333">直播列表 </h3> -->
                            <ul class="mw-tabs">
                                <li @click="playType='lplay'" class="mw-tab" :class="{'active':playType=='lplay'}">直播管理</li>
                              <router-link to="/videoCenter">
                                <li class="mw-tab" :class="{'active':playType=='onplay'}"><!-- @click="playType='onplay'"-->
                                  点播管理
                                </li>
                              </router-link>
                            </ul>
                            <router-link  tag="div" to="/main/newLive1?from=2" v-if="playType=='lplay'">
                                <span class="bgc_txt update_btn">新建直播</span>
                            </router-link>
                        </div>
                        <div>
                          <div  v-if="playType=='lplay'">
                        ...
                        <el-row class="live-wrap">
                            <!-- <el-col :span="8" class="mainData-list">
                                <router-link class="add-live cur-p te-c" tag="div" to="/newLive1">
                                    <span class="el-icon-plus"></span><br>
                                    新建直播频道
                                </router-link>
                            </el-col> -->
                            <el-col v-for="item in dataList" :key="item.id" :span="8" class="mainData-list clear mb30">
                                <div class="mainData_title">
                                    ...
                                    <i class="icon iconfont icon_delete mr16" :disabled="(item,item.no_del)"  @click="edit_live(item,true)"></i>
                                </div>
                                <div class="show_img">
                                                    ...
                                                    <div class="p-a qrcImg">
                                                        <div class="qrcImg-content clear">
                                                            ...
                                                            <div class="fl te-l">
                                                                ...
                                                                <!-- <div><input readonly type="text" style="width: 250px;" v-model="item.playUrl" :id=" 'copyUrl' + item.id" class="el-input__inner"></input></div> -->
                                                                <div><input readonly type="text"  style="width: 375px;margin-bottom:14px;" v-model="playVUrl+item.id" :id=" 'copyUrl' + item.id" class="el-input__inner"></div>
                                                                <div class="mt5"><el-button type="primary" @click="openLiveUrl(playVUrl+item.id)">打开视频</el-button> <el-button class="btn_cc" @click="copyLink(item)">复制地址</el-button></div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                <div class="broadcast-bottom">
                                    <el-col class="broadcast_words">
                                        ...
                                        <!-- {{item.begin_time}}、{{item.notify_begin_time}} -->
                                        <!-- {{item.begin_time || item.notify_begin_time || '暂无直播'}} -->
                            ...
                            <div v-if="noLive" class="no_live">
                                <div class="no_data">
                                    ...
                                    <router-link tag="div" to="/main/newLive1?from=2" class="d-ib  txt_blue">创建直播间</router-link>
...
import {LIVEH5_URL} from "api/config";
  ...
  methods: {
    ...
    qrcanvas(data) {
     ...
     // this.qrcode = PLAY_URL + data.id + "&v=1.7.0";
      this.qrcode=this.playVUrl+data.id;
      console.log("the url is"+this.qrcode);
    ...
    _LiveBroadcast(type, page_NO) {
      ...
      if (type) {
        // url =
        //   DATA_URL +
        //   "mwlive/live/get_listbylivename.do?user_id=" +
        //   getLStorage("user_id") +
        //   "&live_name=" +
        //   this.form.search +
        //   "&page=1&page_size=" +
        ...
        params ={
            ...
            'liveName':this.form.search,
            'page_index':val,
            'page_size':PAGE_SIZE,
            'liveStatus':this.stateOptVal,
            'timeType':this.timeOptVal
        }
        url = DATA_URL + 'mwlive/live/get_listbycreatetime.do';
        // url = DATA_URL + 'mwlive/live/get_listbycreatetime.do?user_id='+ getLStorage('user_id') + '&liveName='+this.form.search+ '&page_index=' + val + '&page_size=' + PAGE_SIZE + '&liveStatus=' + this.stateOptVal+'&timeType=' + this.timeOptVal
        this.isUpload = true;
        if (this.form.search === "") {
          this.isUpload = false;
        }
      ...
      let that = this;
        ...
        success: function(response) {
                if (response.code == 200) {
                    ...
                    let data = response.data.data_list;
                    global.liveData.data = response.data.data_list;
                    ...
                    that.dataList = response.data.data_list;
                    global.liveData.data = that.dataList;
                    that.PageTotal = that.PageDATA.total =
    ...
    //           this.PageTotal = this.PageDATA.total =
    ...
    _StatisticsData() {
      axios
        .get(GIFT_URL + "service/iBacService.action", {
          params: {
            bizCode: 6002,
    ...
    edit_live(event, deleteLive) {
      ...
      this.$router.push({
        path: "/main/newLive1",query: {from: "2"}//1表示云播 2代表视频中心 3代表点播
      });
      return false;
      ...
      axios
        ...
        .then(response => {
          if (response.data.code == ERR_OK) {
            ...
            if (this.page_NO !=1 && this.dataList.length == 1) {
                this._LiveBroadcast(false,this.page_NO-1)
            }else{
                this._LiveBroadcast(false, this.page_NO);
            }
