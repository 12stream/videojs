import {bucketWS,parentFilePathWS,uploadUrlWs,akIdWS,videoRid,map,userRid} from './comm'
var videoRid2  = videoRid
...
function QiniuJsSDK() {
    ...
    function log(type, args){
        var header = "[facebac-js-sdk]["+type+"]";
        var msg = header;
...
//                }
                $.ajax({
            		...
            		success : function(data) {
            			...
            			logger.debug("get new uptoken: ", data.uptoken);
            		},
            		error : function(XMLHttpRequest, textStatus, errorThrown) {
            			logger.error("get uptoken error: ");
            		}
            	});
                return;
...
}else if("video"==fileType2 || "videotape" == fileType2){
...
//			                }
	...
	//		        }
		    ...
		    this.getFileKey = function(FileExt){
		    	var fileNamemd5 = hex_md5("faceBacVideoOut/"+userRid+"/"+videoRid2+"/n_n");
		    	return "store/"+userRid+"/"+videoRid2+"/n_n"+"/"+fileNamemd5+"."+FileExt;

		    };
		    this.getImageKey = function(FileExt){
		    	var fileNamemd5 = hex_md5("faceBacVideoOut/"+userRid+"/"+videoRid2+"/n_n");
		    	return "image/"+userRid+"/"+videoRid2+"/n_n"+"/"+fileNamemd5+"."+FileExt;

		    };
		    this.getRid = function(m){//生成随机数
		    ...
		    this.uploader = function (op) {
		        $("#files").on("click", function (e) {
		        	...
		        	if(tar.hasClass("cx")){
		                ...
		                //var b = localStorage.getItem(parentPath + ":" + fileId);
		        ...
		        that.key_handler = typeof op.init.Key === 'function' ? op.init.Key : '';
		        //this.domain = op.domain;
		        var bucket = bucketWS;//$("#bucketNameId").val()
		        var parentFilePath = parentFilePathWS;//$("#parentPathId").val()
		        var fullFileName = '';
		        var upload_url = uploadUrlWs;//$("#uploadUrlId").val()
		        var akId = akIdWS;//$("#akId").val()
		        var overwrite = 0;
		        ...
		        var getUpToken = function (fileKey) {
		            ...
		            var FileExt=fullFileName.replace(/.+\./,"");   //正则表达式获取后缀
		        	videoRid2 = that.getRid(20);
		        	map[fileKey]=videoRid2;
		        	var key = that.getFileKey(FileExt);
	...
	//	            getUpToken();
		        ...
		        uploader.bind('FileUploaded', (function (_FileUploaded_Handler) {
		            return function (up, file, info) {
		                ...
		                var last_step = function (up, file, info) {
		                    localStorage.removeItem(fullFileName + ":" + file.id);
		                    localStorage.removeItem(fullFileName + ":" + file.id + "_ctx");
		                    var item = $("#fileName_" + file.id);
		                    if (file.percent >= 100) {
		                        //item.html("<span class='upload_success'>上传完成</span>");
		                        item.parents("tr").find(".filectrl_td").remove();
		                    }
		                };
		                var res;
		                var ctx = localStorage.getItem(fullFileName + ":" + file.id + "_ctx");
		...
		global.UploadDemo = UploadDemo;
		})( window );
}
export{fileType2,UploadDemo,Qiniu}
